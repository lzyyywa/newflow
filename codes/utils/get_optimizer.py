...
def get_optimizer_vlm(cfg,model):
    ...
    # ✅ 1. 新增一个列表，专门装 Flow 和 Composer 的参数
    flow_params = [] 
    ...
    if cfg.text_encoding_manner=='composition':
        ...
        for name, param in model.named_parameters():
            if 'c2c' in name:
                c2c_with_wd.append(param)
            # ✅ 2. 把流匹配和 Composer 的参数捕获进来
            elif 'flow' in name or 'composer' in name:
        ...
        optimizer = torch.optim.AdamW([
            ...
            # ✅ 3. 把 Flow 参数喂给优化器！（学习率和 C2C 保持一致）
            {'params': flow_params, 'lr': cfg.visual_lr, 'weight_decay': cfg.visual_wd},
            ],
            betas=(0.9, 0.999), lr=cfg.visual_lr, eps=1e-8,
            weight_decay=cfg.visual_wd)
    else:
